...
import uk.ac.ebi.quickgo.data.OntologySourceFiles;
import uk.ac.ebi.quickgo.data.OntologySourceFiles.ECrossOntologyRelation;
import uk.ac.ebi.quickgo.data.OntologySourceFiles.EFundingBody;
import uk.ac.ebi.quickgo.data.OntologySourceFiles.ETerm;
import uk.ac.ebi.quickgo.data.OntologySourceFiles.ETermComment;
import uk.ac.ebi.quickgo.data.OntologySourceFiles.ETermCredit;
import uk.ac.ebi.quickgo.data.OntologySourceFiles.ETermDefinition;
import uk.ac.ebi.quickgo.data.OntologySourceFiles.ETermDefinitionXref;
import uk.ac.ebi.quickgo.data.OntologySourceFiles.ETermHistory;
import uk.ac.ebi.quickgo.data.OntologySourceFiles.ETermRelation;
import uk.ac.ebi.quickgo.data.OntologySourceFiles.ETermSubset;
import uk.ac.ebi.quickgo.data.OntologySourceFiles.ETermSynonym;
import uk.ac.ebi.quickgo.data.OntologySourceFiles.ETermXref;
import uk.ac.ebi.quickgo.ontology.go.TaxonConstraintSet;
import uk.ac.ebi.quickgo.util.CV;
import uk.ac.ebi.quickgo.util.ColourList;
...
public abstract class GenericOntology implements ITermContainer {
    ...
    public void load(OntologySourceFiles sourceFiles, String rootId) throws Exception {
    	if (sourceFiles.terms != null) {
            for (String[] row : sourceFiles.terms.reader(ETerm.TERM_ID, ETerm.NAME, ETerm.IS_OBSOLETE)) {
                addTerm(new GenericTerm(row[0], row[1], row[2]));
            }
    	}

    	if (sourceFiles.relations != null) {
        	for (String[] row : sourceFiles.relations.reader(ETermRelation.CHILD_ID, ETermRelation.PARENT_ID, ETermRelation.RELATION_TYPE)) {                 

...
public class AnnotationStatisticsService implements StatisticsService {
     ...
     * {@link StatisticsConverter#convert(Stream)} method.
     */
    private class StatisticsConverter {
        ...
        public StatisticsGroup convert(Stream<Aggregation> aggregations) {
            StatisticsGroup statsGroup = new StatisticsGroup(groupName, totalHits);

            aggregations.map(this::createStatsType)
        ...
        private StatisticsByType createStatsType(Aggregation aggregation) {
            ...
            Stream<AggregationBucket> buckets = aggregation.getBuckets();

            buckets.map(this::createStatsValue)
                    .forEach(value -> {
                        if (value.isPresent()) {

...
import ebi.ac.uk.quickgo.Headertypedef;
import ebi.ac.uk.quickgo.ObjectFactory;
import ebi.ac.uk.quickgo.Obotypedef;
import ebi.ac.uk.quickgo.Synonymtypedef;
...
public class GOTermXMLEntityToXMLStream implements XMLEntityToStream<XMLEntity> {
	...
	public void convert(XMLEntity xmlEntity, Format format, OutputStream outputStream) throws JAXBException {
		...
		if(format == Format.OBOXML){
			...
			headertypedef
					.setRemark("This is not the master copy of the OBO files, see http://www.geneontology.org/GO.downloads.ontology.shtml");
			...
			ebi.ac.uk.ws.quickgo.ObjectFactory objectFactory = new ebi.ac.uk.ws.quickgo.ObjectFactory();
			
			JAXBContext context = JAXBContext.newInstance("ebi.ac.uk.ws.quickgo");
			JAXBElement<ebi.ac.uk.ws.quickgo.Lookupdeftype> element = objectFactory.createLookup((ebi.ac.uk.ws.quickgo.Lookupdeftype) ((GOTermXML)xmlEntity).getXmlRepresentation());;
			Marshaller marshaller = context.createMarshaller();		
			marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
			marshaller.setProperty(Marshaller.JAXB_FRAGMENT, true);
			marshaller.marshal( element, outputStream);	

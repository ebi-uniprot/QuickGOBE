...
public class StatisticServiceImpl implements StatisticService{
	...
	public Set<StatsTerm> statisticsByAnnotation(String query, String field) {
		...
		for (Count term : terms) {			
			statsTerms.add(new StatsTerm(term.getName(), statisticsUtil.getName(term.getName(), field), statisticsUtil.calculatePercentage(term.getCount(), numberAnnotations),term.getCount()));			
		}
	...
	public Set<StatsTerm> statisticsByProtein(String query, String field) {
		...
		List<Count> counts = annotationService.getFacetFields(query, null, field, 80);// Facet by field
	...
	private Set<StatsTerm> statisticsByProteinInParallel(String field, String query, List<Count> counts) {
		...
		// Thread 1
		StatisticsThread statsThread1 = createThread(field, query, counts1);		
		statsThread1.start();
		
		// Thread 2
		StatisticsThread statsThread2 = createThread(field, query, counts2);
		statsThread2.start();
		
		// Thread 3
		StatisticsThread statsThread3 = createThread(field, query, counts3);		
		statsThread3.start();
		
		// Thread 4
		StatisticsThread statsThread4 = createThread(field, query, counts4);
		statsThread4.start();
	 ...
	 */
	private StatisticsThread createThread(String field, String query, List<Count> counts){
	
		StatisticsThread statsThread = new StatisticsThread();
		statsThread.setAnnotationService(annotationService);
		statsThread.setStatsUtil(statisticsUtil);
		statsThread.setField(field);
		statsThread.setQuery(query);
		statsThread.setCounts(counts);
		statsThread.setNumberProteins(numberProteins);

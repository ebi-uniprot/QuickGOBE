 ...
 *
 */
public class GenericTerm implements Comparable<GenericTerm> {
    public String id;
    public String name;
    ...
    public GenericTerm(String id, String name, String isObsolete) {
        ...
        this.ontologyType = extractOntologyTypeFromId(id);
    }

    private String extractOntologyTypeFromId(String id) {
    ...
    public void addCrossOntologyRelation(String relation, String foreignNamespace, String foreignID, String foreignTerm,
            String url) {
    ...
    /**
     * return the set of terms that feature somewhere in this term's ancestry, as traversed over a specified set of
     * relation type
     *
    ...
    public List<GenericTerm> getAncestry(String relationCodes) {
        ...
        if (anc == null) {
            ...
            anc = getFilteredAncestors(
                    (relationCodes != null && relationCodes.trim().length() > 0) ? RelationType.toSet(relationCodes) :
                            TermRelation.defaultRelationTypes());
    ...
    public String getAltIdsString() {
        return altIds.stream().map(XRef::getId).collect(Collectors.joining(","));
    }

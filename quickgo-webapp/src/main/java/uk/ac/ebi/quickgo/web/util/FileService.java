...
public class FileService {
    ...
    public StringBuffer generateFastafile(String query, long numberAnnotations, int numResults) {
        ...
        List<Count> counts =
                annotationService.getFacetFields(query, null, AnnotationField.DBOBJECTID.getValue(), numResults);
     ...
     * @return String buffer with the annotations information
     */
    public StringBuffer generateTSVfile(FILE_FORMAT format, String header, String query, long numberAnnotations,
            AnnotationColumn[] columns, int numResults) {
        StringBuffer writer = new StringBuffer();
        int rows = NUM_ROWS;
...
/*
        AnnotationColumn[] columns = {
				AnnotationColumn.DATABASE, AnnotationColumn.PROTEIN, AnnotationColumn.SYMBOL, AnnotationColumn
				.QUALIFIER,
...
*/
            ...
            for (AnnotationColumn annotationColumn : columns) {
                ...
                switch (annotationColumn) {
                    ...
                    case EVIDENCE:
                        ...
                        //						annotationString = annotationString + annotation.getGoEvidence() +
						// separator;
                        //					} else if(format == FileService.FILE_FORMAT.GPAD){
                        //						annotationString = annotationString + annotation.getEcoID() +
						// separator;
                        //					}
                    ...
                    case SYNONYM:
                        ...
                        if (annotation.getDbObjectSynonyms() != null) {
                            synonymString =
                                    StringUtils.arrayToDelimitedString(annotation.getDbObjectSynonyms().toArray(), "|");
                        }
    ...
    public StringBuffer generateJsonForCOOccurrenceStats(String termId, List<COOccurrenceStatsTerm> allStats,
            List<COOccurrenceStatsTerm> nonIEAStats) {
        StringBuffer buffer = new StringBuffer();

        try {
            CooccuringStatsJson statsJson = new CooccuringStatsJson(termId, allStats, nonIEAStats);
            buffer.append(generateJsonFile(statsJson));
        } catch (Exception e) {
            logger.error(e.getMessage());
        }
        return buffer;

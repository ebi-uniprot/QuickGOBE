...
import uk.ac.ebi.quickgo.util.ColourUtils;
...
public class OntologyGraph extends GenericGraph<TermNode, RelationEdge> {
    ...
    public OntologyGraph(EnumSet<RelationType> relationTypes, int ancestorLimit, int pixelLimit, GraphPresentation style) {
    ...
    public GraphImage layout() {
    	GraphImage image;
        if (overflow > 0) {
	        image = new GraphImage("Chart too large - there are more than " + ancestorLimit + " ancestor terms (overflow = " + overflow + ")");
        }
        else {
            GraphLayout<TermNode, RelationEdge> layout = new GraphLayout<TermNode, RelationEdge>(this, GraphLayout.Orientation.TOP);
    ...
    public static OntologyGraph makeGraph(GenericTermSet terms, EnumSet<RelationType> relationTypes, int ancestorLimit, int pixelLimit, GraphPresentation style) {
		...
		for (GenericTerm term : terms.getTerms()) {
			...
			if (node != null && style.fill) {
				...
				node.setFillColour(new Color(colour.length() == 0 ? 0xffffcc : ColourUtils.intDecodeColour("#" + colour)));
			}
		}
    ...
    public static OntologyGraph makeGraph(GenericTermSet terms, int ancestorLimit, int pixelLimit, GraphPresentation style) {
